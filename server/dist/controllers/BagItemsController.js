...
export const addItemToBag = async (req, res, next) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(req.user._id, { $push: { bag: newBag } }, { new: true }).populate("bag");
...
export const deleteBagItem = async (req, res, next) => {
    ...
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(req.user._id, { $pull: { bag: id } }, { new: true }).populate("bag");
        res.send(updatedUser);
    }
    catch (error) {
        next(error);
...
export const checkout = async (req, res, next) => {
    try {
        ...
        await BagHistory.insertMany(userBag?.map((bagItem) => ({
